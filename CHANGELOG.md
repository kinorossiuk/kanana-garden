...
UIS7862S 실장비에서 확인된 볼륨 무반응과 LTE 설정 사용성을 수정한 릴리스입니다.
Python 패키지 버전은 `0.2.2`입니다.

### 포함

- Android `STREAM_MUSIC`과 별개인 FYT MCU 앰프 볼륨을 고정된
  `com.syu.ms` 사운드 모듈로 제어
- 볼륨 올림·내림·음소거·해제와 0~100% 설정을 FYT 명령으로 변환
- FYT 모듈이 없는 일반 Android 장비에서는 활성 볼륨 API로 안전하게 폴백
- 셸·루트·접근성 서비스 없이 고정 package/component와 Binder 명령만 허용
- 저장된 LTE 주소·코드를 기본적으로 접고 `LTE 전송 준비됨` 상태만 표시
- v0.2.1의 LTE 설정을 OTA 업데이트 후 그대로 사용해 평소에는 전송 버튼만 조작
- 앱 화면 `0.2.2`와 GitHub 태그 `v0.2.2`를 보고서 버전과 일치

### 실장비 확인 필요

- v0.2.1에서 길안내·MediaSession·LTE 제출은 통과했고 일반 Android 볼륨 API만
  실제 앰프에 반영되지 않았다.
- v0.2.2 설치 후 FYT 메인 볼륨 올리기와 30% 설정을 다시 확인해야 한다.
