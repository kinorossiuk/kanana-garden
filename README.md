...
기기 진단에는 제조사·모델 외에 Android 보안 패치, hardware·board·ABI와 build
fingerprint를 포함하지만 장비 serial은 수집하지 않습니다.

배포 APK는 `debuggable`을 켜지 않은 release 빌드입니다. 대신 앱 프로세스의
미처리 예외를 최대 12KB, 앱에서 처리한 API·OTA·LTE 내부 오류를 최대 16KB의
앱 비공개 기록으로 보존합니다. 다음 실행 또는 오류 직후 보고서에 stack trace를
자동 포함하며, Android의 원래 crash 처리를 막지 않고 토큰·인증 문자열 형태는
저장 전에 마스킹합니다.
...
uis7862s-report-pull         USB/ADB 보조 경로로 앱 테스트 보고서 수집
report-receiver              LTE/HTTPS 제출 보고서를 현재 저장소에 수신
...
- 앱 비공개 crash 기록과 다음 실행 보고서 자동 첨부 추가
